...
AutoUpdate 是一个为 Minecraft 1.20.1 设计的 Fabric 模组，它允许根据提供的模组列表自动更新模组。通过这个模组，服务器所有者或整合包制作者可以轻松分发更新的模组列表，客户端可以使用这些列表自动同步他们的模组。
...
2. 输入模组列表的版本标识符
3. 点击"导出"
4. 模组列表将以 JSON 文件形式保存在 AutoUpdate 目录中

### 配置选项

模组会在版本文件夹下 `/autoupdate/config.json` 创建配置文件，包含以下选项：
